...
struct ClipboardOffer_body : public packet_head
{
    ClipboardOffer_body():packet_head()
    {
        cmd = ClipboardOffer;
        len = sizeof(ClipboardOffer_body);
    }
    uint32_t size;
};

struct ClipboardRequest_body : public packet_head
{
    ClipboardRequest_body():packet_head()
    {
        cmd = ClipboardRequest;
        len = sizeof(ClipboardRequest_body);
    }
};

struct ClipboardData_body : public packet_head
{
    ClipboardData_body():packet_head()
    {
        cmd = ClipboardData;
        len = sizeof(ClipboardData_body);
    }
    uint32_t size;
};

struct FileOffer_body : public packet_head
{
    FileOffer_body():packet_head()
    {
        cmd = FileOffer;
        len = sizeof(FileOffer_body);
    }
    uint32_t size;
};
#pragma pack(pop)